...
const  AddExpensesTypePage = () => {
 ...
 if (isLoading) {
   ...
   const handleSubmitAdd = async (event) => {
    ...
    setIsLoading(true);
    try {
        const response = await axios.post('https://transitstation.online/api/admin/expence/addtype',formData, {
            headers: {
        ...
        if (response.status === 200) {
            auth.toastSuccess('Expence Type added successfully!');
            handleGoBack();
        } else {
            auth.toastError('Failed to add Expence Type.');
        }
    } catch (error) {
...
export default AddExpensesTypePage
