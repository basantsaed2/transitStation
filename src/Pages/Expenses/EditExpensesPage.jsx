...
import { ExpensesDataContext } from '../../Layouts/EditExpensesLayout';

const EditExpensesPage = () => {
       const expensesContent = useContext(ExpensesDataContext);
    ...
    const handleSelectType = (e) => {
     ...
     const handleClickOutside = (event) => {
       ...
       const handleSubmitEdit = async (expensesId, event) => {
              ...
              setIsLoading(true);
              try {
                  const response = await axios.put(`https://transitstation.online/api/admin/expence/update/${expensesId}`,formData, {
                      headers: {
                          Authorization: `Bearer ${auth.user.token}`,
                  ...
                  if (response.status === 200) {
                      auth.toastSuccess('Expence Updated successfully!');
                      handleGoBack();
                  } else {
                      auth.toastError('Failed to Updated Expence.');
                  }
              } catch (error) {
       ...
       return (
              <>
              <form onSubmit={(event) => handleSubmitEdit(expensesContent.id, event)} className="w-full flex flex-col items-center justify-center gap-y-10">
