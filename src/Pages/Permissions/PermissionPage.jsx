...
const PermissionPage = () => {
    ...
    useEffect(() => {
       ...
       const handleOpenDialog = (positionsId) => {
              setOpenDialog(positionsId);
       };
       ...
       const handleDelete = async (positionsId) => {
              setIsDeleting(true);
              const success = await deletePosition(positionsId, auth.user.token);
              setIsDeleting(false);
              handleCloseDialog();

              if (success) {
                     setPositionsChanged(!positionsChanged)
                     auth.toastSuccess('Role deleted successfully!');
                     setPositions((prevPosition) =>
                            prevPosition.filter((positions) => positions.id !== positionsId)
                     );
              } else {
                     auth.toastError('Failed to delete Role.');
              }
       };

       const deletePosition = async (positionsId, authToken) => {
              try {
                     const response = await axios.delete(`https://transitstation.online/api/admin/deleteadminposition/${positionsId}`, {
                            headers: {
                     ...
                     if (response.status === 200) {
                            ...
                            console.error('Failed to delete Role:', response.status, response.statusText);
                            return false;
                     }
              } catch (error) {
                      ...
                      <table className="w-full sm:min-w-0">
                          <thead className="w-full">
                              <tr className="w-full border-b-2">
                                  ...
                                  <th className="min-w-[150px] sm:w-2/12 lg:w-2/12 text-mainColor text-center font-medium text-sm sm:text-base lg:text-lg xl:text-xl pb-3">Role Name</th>
                                  <th className="min-w-[150px] sm:w-2/12 lg:w-2/12 text-mainColor text-center font-medium text-sm sm:text-base lg:text-lg xl:text-xl pb-3">Role Permission</th>
                                  {(Premission.includes("edit role") ||Premission.includes("delete role"))  && ( 
                          ...
                          <tbody className="w-full">
                                  {positions.map((position, index) => (
                                      <tr className="w-full border-b-2" key={position.id}>
                                              <td
                                             ...
                                             {(Premission.includes("edit role") ||Premission.includes("delete role"))  && ( 
                                              ...
                                              >
                                                      ...
                                                      {Premission?.includes('edit role') && (
                                                      <Link to={`edit/${position.id}`} state={position.id} type="button">
                                                              <EditIcon />
                                                      </Link>
                                                      )}
                                                      {Premission?.includes('delete role') && (
                                                      <button type="button" onClick={() => handleOpenDialog(position.id)}>
                                                              <DeleteIcon />
                                                      </button>
                                                      )}
                                                      {openDialog === position.id && (
                                                                                              ...
                                                                                              <div className="flex items-center">
                                                                                                      <div className="mt-2 text-center">
                                                                                                              <DialogTitle as="h3" className="text-xl font-semibold leading-10 text-gray-900">
                                                                                                                      You will delete role {position.name|| "null"}
                                                                                                              </DialogTitle>
                                                                                                      </div>
                                                                                              </div>
                                                                                      </div>
                                   ...
                                   {openView === position.id && (
                                          <Dialog open={true} onClose={handleCloseView} className="relative z-10">
                                                 ...
                                                 {position.role.length === 0 ? (
                                                        ...
                                                        position.role.map((permission, index) => {
                                                               ...
                                                               return (
                                                                      ...
                                                                      >
                                                                             <span className="text-mainColor text-lg lg:text-xl font-semibold capitalize">
                                                                                    {displayIndex}. {permission.role_name}
                                                                             </span>
                                                                      </div>
                                                               );
                                                        })
