...
const AddRevenuePage = () => {
   ...
   const handleSubmitAdd = async (event) => {
       ...
       setIsLoading(true);
       try {
           const response = await axios.post('https://transitstation.online/api/admin/revenue/add',formData, {
               headers: {
                   Authorization: `Bearer ${auth.user.token}`,
           ...
           if (response.status === 200) {
               auth.toastSuccess('Revenue added successfully!');
               handleGoBack();
           } else {
               auth.toastError('Failed to add Revenue.');
           }
       } catch (error) {
