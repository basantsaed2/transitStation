...
const  RevenueTypePage = () => {

    const auth = useAuth();
    const [Premission] = useState(auth.user.permissions.role);
    const [isLoading, setIsLoading] = useState(false);
    const [revenueType, setRevenueType] = useState('');
    const [revenueTypeChanged, setRevenueTypeChanged] = useState(false);
    ...
    const fetchData = async () => {
        ...
        try {
               ...
               if (response.status === 200) {
                      ...
                      setRevenueType(response.data.key)
               }
        } catch (error) {
    ...
    const handleOpenDialog = (typeId) => {
       ...
       const handleDelete = async (typeId) => {
              setIsDeleting(true);
              const success = await deleteRevenueType(typeId, auth.user.token);
              setIsDeleting(false);
              handleCloseDialog();

              if (success) {
                     setRevenueTypeChanged(!revenueTypeChanged)
                     auth.toastSuccess('Revenue Type deleted successfully!');
                     setRevenueType((prevRevenueType) =>
                        prevRevenueType.filter((type) => type.id !== typeId)
                     );
              } else {
                     auth.toastError('Failed to delete Revenue Type.');
              }
       };

       const deleteRevenueType = async (typeId, authToken) => {
              try {
                     const response = await axios.delete(`https://transitstation.online/api/admin/revenue/types/delete/${typeId}`, {
                            headers: {
                     ...
                     if (response.status === 200) {
                            console.log('Revenue Type deleted successfully');
                            return true;
                     } else {
                            console.error('Failed to delete Revenue Type:', response.status, response.statusText);
                            return false;
                     }
              } catch (error) {
...
//     if (!revenueType) {
//         return <div className='text-mainColor text-2xl font-bold w-full h-full flex items-center justify-center'>No Revenues Type data available</div>;
//     }
       ...
       return (
        ...
        <div className="w-full">
            <div className="w-full flex flex-wrap items-center justify-start gap-10">
             ...
             {Premission.includes('add revenue_type') && (
             <Link to={'add'}>
                ...
                <table className="w-full sm:min-w-0">
                    <thead className="w-full">
                        <tr className="w-full border-b-2">
                            ...
                            <th className="min-w-[150px] sm:w-2/12 lg:w-2/12 text-mainColor text-center font-medium text-sm sm:text-base lg:text-lg xl:text-xl pb-3">Revenue Type</th>
                            {(Premission.includes("edit revenue_type") ||Premission.includes("delete revenue_type"))  && ( 
                            ...
                            {revenueType.map((type, index) => (
                                ...
                                <tr className="w-full border-b-2" key={type.id}>
                                        ...
                                        >
                                                ...
                                                {Premission?.includes('edit revenue_type') && (
                                                <Link to={`edit/${type.id}`} state={type.id} type="button">
                                                        <EditIcon />
                                                </Link>
                                                )}
                                                {Premission?.includes('delete revenue_type') && (
                                                                                        ...
                                                                                        <div className="flex items-center">
                                                                                                <div className="mt-2 text-center">
                                                                                                        <DialogTitle as="h3" className="text-xl font-semibold leading-10 text-gray-900">
                                                                                                                You will delete revenue type {type.type_name|| "null"}
                                                                                                        </DialogTitle>
                                                                                                </div>
                                                                                        </div>
                                                                                </div>
...
export default RevenueTypePage
