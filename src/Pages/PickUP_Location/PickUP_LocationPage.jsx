      ...
      const deleteLocation = async (locationId, authToken) => {
             try {
                    ...
                    if (response.status === 200) {
                           ...
                           console.error('Failed to delete Location:', response.status, response.statusText);
                           return false;
                    }
             } catch (error) {
...
<div className='w-full flex flex-wrap gap-10'>
      {/* Location list */}
      {pickUp_Locations.map((location) => (
        <div key={location.id} className='w-full lg:w-[45%] p-4 pr-0 flex lg:flex-row sm:flex-col bg-[#FFF979] rounded-lg shadow-lg mb-4'>
          ...
          <div className="relative w-3/12">
            ...
            <button onClick={() => toggleDropdown(location.id)} className="absolute top-2 right-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-black font-bold text-xl" fill="none" viewBox="0 0 24 24" stroke="currentColor">
