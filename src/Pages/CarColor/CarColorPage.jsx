...
const CarColorPage = () => {
    ...
    const handleOpenDialog = (colorId) => {
        ...
        const handleDelete = async (colorId) => {
               setIsDeleting(true);
               const success = await deleteColor(colorId, auth.user.token);
               setIsDeleting(false);
               handleCloseDialog();
 
               if (success) {
                      setColorChanged(!colorChanged)
                      auth.toastSuccess('Color deleted successfully!');
                      setColors((prevColor) =>
                        prevColor.filter((color) => color.id !== colorId)
                      );
               } else {
                      auth.toastError('Failed to delete Color.');
               }
        };
 
        const deleteColor = async (colorId, authToken) => {
               try {
                      const response = await axios.delete(`https://transitstation.online/api/admin/color/delete/${colorId}`, {
                             headers: {
                      ...
                      if (response.status === 200) {
                             ...
                             console.error('Failed to delete Color:', response.status, response.statusText);
                             return false;
                      }
               } catch (error) {
        ...
        // if (!colors) {
        //     return <div className='text-mainColor text-2xl font-bold w-full h-full flex items-center justify-center'>No Colors Car data available</div>;
        // }
          ...
          colors.length === 0 ? (
                ...
                <div className="w-full flex items-center justify-between mt-4 overflow-x-auto">
                        ...
                        <thead className="w-full">
                                ...
                                <th className="min-w-[150px] sm:w-2/12 lg:w-2/12 text-mainColor text-center font-medium text-sm sm:text-base lg:text-lg xl:text-xl pb-3">Color Name</th>
                                <th className="min-w-[150px] sm:w-2/12 lg:w-2/12 text-mainColor text-center font-medium text-sm sm:text-base lg:text-lg xl:text-xl pb-3">Color Image</th>
                                {(Premission.includes("edit color") ||Premission.includes("delete color"))  && ( 
                        ...
                        <tbody className="w-full">
                                {colors.map((color, index) => (
                                        <tr className="w-full border-b-2" key={color.id}>
                                                <td
                                                ...
                                                >
                                                        {color?.color_code ? (
                                                <div
                                                        className="w-8 h-8 inline-block rounded-full border border-gray-300"
                                                        style={{ backgroundColor: color.color_code }} // Dynamically apply the color from API
                                                />
                                                                ) : (
                                                                'Null'
                                                        )}
                                                ...
                                                >
                                                        ...
                                                        {Premission?.includes('edit color') && (
                                                        <Link to={`edit/${color.id}`} state={color.id} type="button">
                                                                <EditIcon />
                                                        </Link>
                                                        )}
                                                        {Premission?.includes('delete color') && (
                                                        <button type="button" onClick={() => handleOpenDialog(color.id)}>
                                                                <DeleteIcon />
                                                        </button>
                                                        )}
                                                        {openDialog === color.id && (
                                                                                                ...
                                                                                                <div className="flex items-center">
                                                                                                        <div className="mt-2 text-center">
                                                                                                                <DialogTitle as="h3" className="text-xl font-semibold leading-10 text-gray-900">
                                                                                                                        You will delete color {color.color_name|| "null"}
                                                                                                                </DialogTitle>
                                                                                                        </div>
                                                                                                </div>
                                                                                        </div>
