...
import { SubscriptionsDataContext } from '../../Layouts/EditSubscriptionsLayout';

const EditSubscriptionsPage = () => {
       const subscriptionContent = useContext(SubscriptionsDataContext);
   ...
   const handleSubmitEdit = async (subscriptionId,event) => {
       ...
       try {
           ...
           if (response.status === 200) {
               auth.toastSuccess('Subscriptionnue Updated successfully!');
               handleGoBack();
           } else {
               auth.toastError('Failed to Updated Subscription.');
           }
       } catch (error) {
       ...
       return (
              <>
              <form onSubmit={(event) => handleSubmitEdit(subscriptionContent.id, event)} className="w-full flex flex-col items-center justify-center gap-y-10">
