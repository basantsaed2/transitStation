            ...
            const handleClickOutside = (event) => {
              ...
              const handleSubmitAdd = async (event) => {
                 ...
                 if (!selectPositionId) {
                     ...
                     setIsLoading(true);
                     try {
                         const response = await axios.post('https://transitstation.online/api/admin/addadmin',formData, {
                             headers: {
                                 Authorization: `Bearer ${auth.user.token}`,
                         ...
                         if (response.status === 200) {
                             auth.toastSuccess('Supervisor added successfully!');
                             handleGoBack();
                         } else {
                             auth.toastError('Failed to add Supervisor.');
                         }
                     } catch (error) {
