...
import { RequestDataContext } from '../../Layouts/EditRequestLayout';

const EditRequestPage = () => {
    const requestContent = useContext(RequestDataContext);
    ...
    useEffect(() => {
       if (requestContent) {
              ...
              if (requestContent?.user) {
                     ...
                     setSelectUserId(requestContent.user?.id);
              } else {
                     setSelectUser('Select User');
                     setSelectUserId(null);
              }
              
              if (requestContent?.location) {
                     setSelectLocation(requestContent.location?.pick_up_address);
                     setSelectLocationId(requestContent.location?.id);
              } else {
                     setSelectLocation('Select Pick_UP Location');
    ...
    const fetchData = async () => {
     ...
     const handleOpenSelectCar = () => {
      ...
      const handleClickOutside = (event) => {
        ...
        const handleSubmitEdit = async (requestId ,event) => {
            ...
            setIsLoading(true);
            try {
                const response = await axios.put(`https://transitstation.online/api/admin/request/update/${requestId}`,formData, {
                    headers: {
                        Authorization: `Bearer ${auth.user.token}`,
                ...
                if (response.status === 200) {
                    auth.toastSuccess('Request Updated successfully!');
                    handleGoBack();
                } else {
                    auth.toastError('Failed to Updated Request.');
                }
            } catch (error) {
       ...
       return (
              <form onSubmit={(event) => handleSubmitEdit(requestContent.id, event)} className="w-full flex flex-col items-center justify-center gap-y-10">
